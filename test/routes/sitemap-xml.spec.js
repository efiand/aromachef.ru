...
describe('Routes/Sitemap-xml', () => {
	...
	test('Returns xml sitemap with service page urls', async () => {
		...
		assert.match(xml, new RegExp(`<loc>${BASE_URL}/privacy</loc>`));
		assert.match(xml, new RegExp(`<loc>${BASE_URL}/structure</loc>`));
	});
});
